from numpy import sum     #import some important library
from typing import TextIO
import os


def read_tasks_times() -> dict[int, int]:      #read tasks time file
    tasks_times = {}
...
def read_links(tasks_times: dict[int, int]) -> dict[int, list[int]]:   #read links between tasks file
    links = {}
...
#this class to calculate elements of each step,total time for this element and diff for the element
class ProductionLine:  
    ...
    def write(self, writable_file: TextIO) -> None:    #write the results in text file
        writable_file.write(
            f"{self.idx} : {self.tasks_ids} Total Time=({self.total_time}), dif={round(self.diff, 2)} \n"
        )


class Solution:         #this class to calculate all results for each solution
    def __init__(
        self,
        ...
        for production_line in production_lines:  #for loop to calculate min time and max time and total differences
        ...
        self.average_diff = self.max_time / len(self.production_lines)   #calculate average differences

    def write(self, writable_file: TextIO) -> None:       #function to write all solution in text file
...
#calculate every acceptable sorted tasks 
...
#calculate every cuts that compatiable with sorted tasks
...
def get_all_solutions(             #save all solution in a list
    tasks_times: dict[int, int], links: dict[int, list[int]], steps: int
) -> list[Solution]:
    total_time = sum([task_time for task_time in tasks_times.values()], dtype=int) #calculate total time
    average_time = total_time / steps                                          #calculate average time
    ...
    solutions = []        #list to save the solutions
...
#function to get best solution by the minimum total differences
...
#function to write all solution in text file
...
#function to write best solution in text file
def write_solution(solution: Solution, file_name: str) -> None:
    with open(file_name, "w") as writable_file:
        solution.write(writable_file)

#call the functions by order
def main():
